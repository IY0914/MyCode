...
void UserInterface::LoadTexture(const wchar_t* path)
{
	...
	DirectX::CreateWICTextureFromFile(m_deviceResources->GetD3DDevice(), path, m_res.ReleaseAndGetAddressOf(), m_tex.ReleaseAndGetAddressOf());
...
void UserInterface::CreateShader()
{
	...
	BinaryFile VSData = BinaryFile::LoadFile(L"Resources/Shader/UserInterfaceVS.cso");
	BinaryFile GSData = BinaryFile::LoadFile(L"Resources/Shader/UserInterfaceGS.cso");
	BinaryFile PSData = BinaryFile::LoadFile(L"Resources/Shader/UserInterfacePS.cso");
